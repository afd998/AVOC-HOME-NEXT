...
import { SidebarMenuButton } from "@/app/(dashboard)/sidebar/sidebar";
...
const NotificationsModal: React.FC<> = () => {
  ...
  const hasStaffAssistance = (event: Event): boolean => {
    ...
    return event.resources.some(
      (item: any) => item.itemName === "KSM-KGH-AV-Staff Assistance"
    );
  };
      ...
      if (events) {
        events.forEach((event) => {
          ...
          if (notificationTime > now) {
            const timeUntilNotification = Math.floor(
              (notificationTime.getTime() - now.getTime()) / (1000 * 60)
            );
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Notifications for events with staff assistance (10 minutes before
            event)
            <br />
            <span className="text-xs text-muted-foreground">
              Only showing events assigned to you from notification-enabled
              rooms
            </span>
          </DialogDescription>
        </DialogHeader>
              ...
              {scheduledNotifications.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground">
                    No upcoming events with staff assistance assigned to you for
                    the next 7 days.
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                  {scheduledNotifications.map((notification, index) => (
                    <div
                      ...
                      }-${notification.notificationTime.getTime()}`}
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                            <div>
                              ...
                              <div className="font-medium">
                                {formatTime(notification.eventStartTime)} -{" "}
                                {formatTime(
                                  new Date(
                                    `${notification.event.date}T${notification.event.end_time}`
                                  )
                                )}
                              </div>
                            ...
                            <div>
                              ...
                              >
                                ...
                                {getTimeUntilNotification(
                                  notification.timeUntilNotification
                                )}
                              </div>
                            </div>
                          </div>

                          {notification.event.instructor_names &&
                            Array.isArray(
                              notification.event.instructor_names
                            ) &&
                            notification.event.instructor_names.length > 0 && (
                              <div className="mt-2 text-sm">
                                <span className="font-medium text-muted-foreground">
                                  Instructor
                                  {notification.event.instructor_names.length >
                                  1
                                    ? "s"
                                    : ""}
                                  :
                                ...
                                <span className="ml-1">
                                  {notification.event.instructor_names
                                    .length === 1
                                    ? String(
                                        notification.event.instructor_names[0]
                                      )
        ...
        <DialogFooter className="flex-col sm:flex-row sm:justify-between">
          <div className="text-sm text-muted-foreground text-center sm:text-left mb-4 sm:mb-0">
            <p>
              Event notifications appear 15 minutes before events with staff
              assistance or web conferencing.
            </p>
            <p className="mt-1">
              Only events in selected notification rooms will trigger alerts.
            </p>
            {scheduledNotifications.length > 0 && (
              <p className="mt-2 font-medium">
                {scheduledNotifications.length} notification
                {scheduledNotifications.length !== 1 ? "s" : ""} scheduled
              </p>
            )}
          </div>
