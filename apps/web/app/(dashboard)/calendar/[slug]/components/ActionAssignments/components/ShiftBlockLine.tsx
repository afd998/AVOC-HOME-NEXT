...
import { ShiftBlock } from '@/features/SessionAssignments/hooks/useShiftBlocks';
...
const ShiftBlockLine: React.FC<ShiftBlockLineProps> = ({ shiftBlock, pixelsPerMinute, pageZoom, allRoomsAssigned }) => {
  ...
  const calculateWidth = () => {
    if (!shiftBlock.startTime || !shiftBlock.endTime) return 0;
    
    const startTime = new Date(`2000-01-01T${shiftBlock.startTime}`);
    const endTime = new Date(`2000-01-01T${shiftBlock.endTime}`);
  ...
  const timeText = `${formatTimeLabel(shiftBlock.startTime)} - ${formatTimeLabel(shiftBlock.endTime)}`;
  ...
  if (typeof window !== 'undefined') {
    console.log('[ShiftBlockLine] block', shiftBlock.id, {
      start: shiftBlock.startTime,
      end: shiftBlock.endTime,
      assignments,
      assignmentProfiles: assignments.map((a: any) => ({
        user: a?.user,
        name: a?.name,
        profile: a?.profile,
      })),
        ...
        <TooltipProvider>
          <div className="flex items-center gap-2">
            <Tooltip>
              ...
              <TooltipContent>
                <p>{formatTimeLabel(shiftBlock.startTime)} - {formatTimeLabel(shiftBlock.endTime)}</p>
              </TooltipContent>
            </Tooltip>
            {allRoomsAssigned && (
              <Tooltip>
