...
type Reservation = NonNullable<
  NonNullable<
    NonNullable<RawEvent["itemDetails"]>["occur"]
  >["prof"]
>[number]["rsv"];
...
export const parseEventResources = (event: RawEvent) => {
  ...
  return matchingReservation.res.map((resource) => ({
    itemName:
      typeof resource.itemName === "string" ? resource.itemName : null,
    quantity:
      typeof resource.quantity === "number" ? resource.quantity : null,
    instruction:
      typeof resource.instruction === "string"
