import { pgTable, pgPolicy, bigint, timestamp, jsonb, time, date, text, boolean, index, foreignKey, check, uuid, unique, smallint, integer, bigserial } from "drizzle-orm/pg-core"
...
export const events = pgTable("events", {
	...
	index("idx_events_date_start_time").using("btree", table.date.asc().nullsLast().op("date_ops"), table.startTime.asc().nullsLast().op("time_ops")),
	index("idx_events_end_time").using("btree", table.endTime.asc().nullsLast().op("time_ops")),
	index("idx_events_event_name_start_time").using("btree", table.eventName.asc().nullsLast().op("text_ops"), table.startTime.asc().nullsLast().op("time_ops")),
	...
	index("idx_events_item_id").using("btree", table.id.asc().nullsLast().op("int8_ops")),
	index("idx_events_item_id2").using("btree", table.itemId2.asc().nullsLast().op("int8_ops")),
...
export const faculty = pgTable("faculty", {
	...
	timing: smallint().default(sql`'1'`),
	complexity: smallint().default(sql`'1'`),
	temperment: integer().default(1),
	usesMic: boolean("uses_mic").default(false),
	leftSource: text("left_source").default('ROOM_PC'),
	rightSource: text("right_source").default('ROOM_PC'),
